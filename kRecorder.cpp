...
kRecorder::WORD kRecorder::ld_word (const BYTE* ptr)	/*	 Load a 2-byte little-endian word */
{
	WORD rv;
...
kRecorder::DWORD kRecorder::ld_dword (const BYTE* ptr)	/* Load a 4-byte little-endian word */
{
	DWORD rv;
...
#if !_FS_READONLY
void kRecorder::st_word (BYTE* ptr, WORD val)	/* Store a 2-byte word in little-endian */
{
	*ptr++ = (BYTE)val; val >>= 8;
	*ptr++ = (BYTE)val;
}
void kRecorder::st_dword (BYTE* ptr, DWORD val)	/* Store a 4-byte word in little-endian */
{
	*ptr++ = (BYTE)val; val >>= 8;
	*ptr++ = (BYTE)val; val >>= 8;
	*ptr++ = (BYTE)val; val >>= 8;
...
void kRecorder::mem_cpy (void* dst, const void* src, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
/* Fill memory block */
void kRecorder::mem_set (void* dst, int val, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
int kRecorder::mem_cmp (const void* dst, const void* src, UINT cnt) {	/* ZR:same, NZ:different */
...
int kRecorder::chk_chr (const char* str, int chr) {	/* NZ:contained, ZR:not contained */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::sync_window (	/* Returns FR_OK or FR_DISK_ERROR */
	FATFS* fs			/* File system object */
)
{
	DWORD wsect;
...
kRecorder::FRESULT kRecorder::move_window (	/* Returns FR_OK or FR_DISK_ERROR */
	FATFS* fs,			/* File system object */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::sync_fs (	/* FR_OK:succeeded, !=0:error */
	FATFS* fs		/* File system object */
)
{
	FRESULT res;
...
kRecorder::DWORD kRecorder::clust2sect (	/* !=0:Sector number, 0:Failed (invalid cluster#) */
	FATFS* fs,		/* File system object */
...
kRecorder::DWORD kRecorder::get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x7FFFFFFF:Cluster status */
...
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::put_fat (	/* FR_OK(0):succeeded, !=0:error */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::remove_chain (	/* FR_OK(0):succeeded, !=0:error */
	_FDID* obj,			/* Corresponding object */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
kRecorder::DWORD kRecorder::create_chain (	/* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
	_FDID* obj,			/* Corresponding object */
...
kRecorder::FRESULT kRecorder::dir_sdi (	/* FR_OK(0):succeeded, !=0:error */
...
kRecorder::FRESULT kRecorder::dir_next (	/* FR_OK(0):succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
...
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::dir_alloc (	/* FR_OK(0):succeeded, !=0:error */
	DIR* dp,		/* Pointer to the directory object */
...
kRecorder::DWORD kRecorder::ld_clust (	/* Returns the top cluster value of the SFN entry */
...
kRecorder::FRESULT kRecorder::dir_read (
	DIR* dp,		/* Pointer to the directory object */
...
kRecorder::FRESULT kRecorder::dir_find (	/* FR_OK(0):succeeded, !=0:error */
...
/*-----------------------------------------------------------------------*/
#if !_FS_READONLY
kRecorder::FRESULT kRecorder::dir_register (	/* FR_OK:succeeded, FR_DENIED:no free entry or too many SFN collision, FR_DISK_ERR:disk error */
...
kRecorder::FRESULT kRecorder::dir_remove (	/* FR_OK:Succeeded, FR_DISK_ERR:A disk error */
...
void kRecorder::get_fileinfo (		/* No return code */
	DIR* dp,			/* Pointer to the directory object */
...
kRecorder::FRESULT kRecorder::create_name (	/* FR_OK: successful, FR_INVALID_NAME: could not create */
	DIR* dp,			/* Pointer to the directory object */
...
kRecorder::FRESULT kRecorder::follow_path (	/* FR_OK(0): successful, !=0: error code */
...
int kRecorder::get_ldnumber (		/* Returns logical drive number (-1:invalid drive) */
...
kRecorder::BYTE kRecorder::check_fs (	/* 0:FAT, 1:exFAT, 2:Valid BS but not FAT, 3:Not a BS, 4:Disk error */
	FATFS* fs,	/* File system object */
...
kRecorder::FRESULT kRecorder::find_volume (	/* FR_OK(0): successful, !=0: any error occurred */
...
kRecorder::FRESULT kRecorder::validate (	/* Returns FR_OK or FR_INVALID_OBJECT */
...
kRecorder::FRESULT kRecorder::f_mount (
...
kRecorder::FRESULT kRecorder::f_open (
...
kRecorder::FRESULT kRecorder::f_read (
	FIL* fp, 	/* Pointer to the file object */
...
kRecorder::FRESULT kRecorder::f_write (
	FIL* fp,			/* Pointer to the file object */
...
kRecorder::FRESULT kRecorder::f_sync (
	FIL* fp		/* Pointer to the file object */
)
{
	FRESULT res;
...
kRecorder::FRESULT kRecorder::f_close (
...
kRecorder::FRESULT kRecorder::f_lseek (
	FIL* fp,		/* Pointer to the file object */
...
kRecorder::FRESULT kRecorder::f_opendir (
...
kRecorder::FRESULT kRecorder::f_closedir (
...
kRecorder::FRESULT kRecorder::f_readdir (
...
kRecorder::FRESULT kRecorder::f_stat (
...
kRecorder::FRESULT kRecorder::f_getfree (
...
kRecorder::FRESULT kRecorder::f_truncate (
	FIL* fp		/* Pointer to the file object */
)
{
	FRESULT res;
...
kRecorder::FRESULT kRecorder::f_unlink (
...
kRecorder::FRESULT kRecorder::f_mkdir (
	const TCHAR* path		/* Pointer to the directory path */
)
{
	FRESULT res;
...
kRecorder::FRESULT kRecorder::f_rename (
...
kRecorder::kRecorder(void)
{
	this->isFileOpen = false;
}
bool kRecorder::init(void)
{
    // zamontuj w systemie dysk logiczny o numerze 0
    // funkcja inicjalizuje jednosczesnie pracę karty SD w tryb SPI
	mRESULT = f_mount(&mFatFs,"0",1);
	if(mRESULT == FR_OK) return true;
	return false;
}
bool kRecorder::openLog(const char * file_path)
{
	// zamknij plik
	this->closeLog();

	// otwórz plik temp.txt do zapisu. Jesli plik nie istnieje utwórz go
	mRESULT = f_open(&mFile,file_path, FA_WRITE | FA_OPEN_ALWAYS);
	if (mRESULT== FR_OK){

		// przestaw kursor na sam początek pliku
		mRESULT = f_lseek(&mFile,0);
		if(mRESULT != FR_OK) return false;
		// obetnij całą zawartosć pliku za kursorem (tutaj czysci cały plik)
		mRESULT = f_truncate(&mFile);
		if(mRESULT != FR_OK) return false;
...
void kRecorder::closeLog(void)
{
	// zamknij plik
	mRESULT = f_close(&mFile);
	this->isFileOpen = false;
}
bool kRecorder::writeRecord(const char * data)
{
	UINT bytesToWrite=0;
	UINT bytesWritten=0;
	const char * p = data;
	if(this->isFileOpen)
	{
		while(*p)
			...
			mRESULT = f_write(&mFile,data,bytesToWrite,&bytesWritten);
			if(mRESULT != FR_OK) return false;
			if(bytesToWrite != bytesWritten) return false;
			return true;
	}
	return false;

}
bool kRecorder::writeRecord(char * buff,unsigned char bytes)
{
	UINT bytesToWrite=(UINT)bytes;
	UINT bytesWritten=0;

	if(this->isFileOpen)
	{
		mRESULT = f_write(&mFile,(const char *)buff,bytesToWrite,&bytesWritten);
		if(mRESULT != FR_OK) return false;
		if(bytesToWrite != bytesWritten) return false;
		return true;
	}
...
void kRecorder::SELECT (void) 		// CS w stan niski
{
	GPIO_ResetBits(GPIOB, GPIO_Pin_6);
}

void kRecorder::DESELECT (void) 	// CS w stan wysoki
{
	GPIO_SetBits(GPIOB, GPIO_Pin_6);
}

void kRecorder::xmit_spi (BYTE Data)  // Wyslanie bajtu do SD
{
  // Wyslanie bajtu
...
kRecorder::BYTE kRecorder::rcvr_spi (void) 		// Odebranie bajtu z SD
{
  u8 Data = 0;
...
void kRecorder::rcvr_spi_m (BYTE *dst)
{
    *dst = rcvr_spi();
}
...
kRecorder::BYTE kRecorder::wait_ready (void)
{
    BYTE res;
...
void kRecorder::power_off (void)
{
    PowerFlag = 0;
}

int kRecorder::chk_power(void)        /* Socket power state: 0=off, 1=on */
{
    return PowerFlag;
}
...
unsigned char kRecorder::rcvr_datablock (
...
unsigned char kRecorder::xmit_datablock (
...
kRecorder::BYTE kRecorder::send_cmd (
...
kRecorder::DSTATUS kRecorder::disk_initialize (
...
kRecorder::DSTATUS kRecorder::disk_status (
    BYTE drv        /* Physical drive nmuber (0) */
)
{
    if (drv) return STA_NOINIT;        /* Supports only single drive */
...
/* Read Sector(s)                                                        */
/*-----------------------------------------------------------------------*/
kRecorder::DRESULT kRecorder::disk_read (
...
#if _READONLY == 0
kRecorder::DRESULT kRecorder::disk_write (
    BYTE pdrv,            /* Physical drive nmuber (0) */
...
kRecorder::DRESULT kRecorder::disk_ioctl (
    BYTE drv,        /* Physical drive nmuber (0) */
...
void kRecorder::disk_timerproc (void)
{
//    BYTE n, s;
    BYTE n;
...
kRecorder::DWORD kRecorder::get_fattime (void)
{
